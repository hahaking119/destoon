...
<td class="tab" id="action_pay"><a href="<?php echo $MOD['linkurl'];?>charge.php?action=pay"><span>在线充值</span></a></td>
<td class="tab_nav">&nbsp;</td>
<?php } ?>
<td class="tab" id="action_card"><a href="<?php echo $MOD['linkurl'];?>charge.php?action=card"><span>充值卡充值</span></a></td>
<?php if($MOD['pay_url']) { ?>
<td class="tab_nav">&nbsp;</td>
<td class="tab"><a href="<?php echo $MOD['pay_url'];?>"><span>银行汇款</span></a></td>
<?php } ?>
<td class="tab_nav">&nbsp;</td>
<td class="tab" id="action_record"><a href="<?php echo $MOD['linkurl'];?>charge.php?action=record"><span>充值记录</span></a></td>
</tr>
</table>
</div>
...
<option value="<?php echo $k;?>" <?php if($bank == $k) { ?>selected<?php } ?>><?php echo $v['name'];?></option>;
<?php } } ?>
</select>
&nbsp;
...
<form method="post" action="<?php echo $MOD['linkurl'];?>charge.php" onsubmit="return check_card();">
...
<?php if($MOD['pay_online']) { ?>
<form method="post" action="<?php echo $MOD['linkurl'];?>charge.php" onsubmit="return check();" id="dform">
...
<?php if($charges) { ?>
<?php if(is_array($charges)) { foreach($charges as $k => $v) { ?>
<input type="radio" name="amount" value="<?php echo $v;?>" id="amount_<?php echo $k;?>"<?php if($k==0) { ?>checked<?php } ?>/><label for="amount_<?php echo $k;?>"> <?php echo $v;?><?php echo $DT['money_unit'];?></label>&nbsp;
<?php } } ?>
<?php } else { ?>
...
<td class="tr">
	...
	<?php if(is_array($PAY)) { foreach($PAY as $k=>$v) { ?>
	<?php if($v['enable']) { ?>
	<tr onclick="$('<?php echo $k;?>').checked=true;">
	<td><input type="radio" name="bank" value="<?php echo $k;?>" id="<?php echo $k;?>"<?php if($i==0) { ?> checked<?php } ?>/></td>
	<td><img src="<?php echo DT_SKIN;?>image/logo_<?php echo $k;?>.gif" alt=""/></td>
...
<?php if($mincharge) { ?>
	if($('amount').value < <?php echo $mincharge;?>) {
...
<form method="post" action="<?php echo $MOD['linkurl'];?>charge.php">
<input type="hidden" name="action" value="confirm"/>
...
<tr>
<td class="tl">支付平台</td>
<td class="tr"><img src="<?php echo DT_SKIN;?>image/logo_<?php echo $bank;?>.gif" alt=""/></td>
</tr>
<tr>
<td class="tl">充值金额</td>
...
<td class="tr lh18">
- 错误代码“<?php echo $charge_errcode;?>”，请与<a href="<?php echo $MOD['linkurl'];?>ask.php?action=add" class="b">客服联系</a>并告知错误代码。<br/>
- <a href="<?php echo $MOD['linkurl'];?>charge.php?action=pay" class="b">重新充值请点这里。</a><br/>
</td>
</tr>
...
<td class="tl" height="50">相关说明</td>
<td class="tr lh18">
- <a href="<?php echo $MOD['linkurl'];?>record.php?action=charge" class="b">查询记录请点这里。</a><br/>
- <a href="<?php echo $MOD['linkurl'];?>charge.php?action=pay" class="b">继续充值请点这里。</a><br/>
</td>
</tr>
<?php } else { ?>
<tr>
...
<td class="tl" height="50">相关说明</td>
<td class="tr lh18">
- 如果您确认充值成功，请立即<a href="<?php echo $MOD['linkurl'];?>ask.php?action=add" class="b">联系客服</a>解决。<br/>
- <a href="<?php echo $MOD['linkurl'];?>charge.php?action=pay" class="b">重新充值请点这里。</a><br/>
</td>
</tr>
<?php } ?>
