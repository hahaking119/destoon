...
<div class="m_l_1 f_l">
	<div class="left_box">		
		...
		<br/>
			<table cellpadding="5" cellspacing="3">
			<?php if(is_array($OP)) { foreach($OP as $p) { ?>
			<tr>
			<td width="80" align="right"><?php echo $p['name'];?>：</td>
			<td>
			<input type="hidden" name="option_<?php echo $p['oid'];?>" id="option_<?php echo $p['oid'];?>" value="<?php echo $p['select'];?>"/>
			<a href="###" onclick="$('option_<?php echo $p['oid'];?>').value='';$('fsearch').submit();" class="t<?php if($p['select']=='') { ?> f_b<?php } ?>">全部</a>
			<?php if(is_array($p['value'])) { foreach($p['value'] as $v) { ?>
			&nbsp;<a href="###" onclick="$('option_<?php echo $p['oid'];?>').value='<?php echo $v;?>';$('fsearch').submit();" class="t<?php if($p['select']==$v) { ?> f_b<?php } ?>"><?php echo $v;?></a>
			<?php } } ?>
			</td>
			</tr>
			<?php } } ?>
		...
		<div class="tool">
			...
			<?php } else { ?>
			<img src="<?php echo DT_SKIN;?>image/list_txt.gif" width="16" height="16" alt="文字列表" align="absmiddle" class="c_p" onclick="$('list').value=2;$('fsearch').submit();"/>&nbsp;
			<?php } ?>
			<?php if($list == 1) { ?>
			<img src="<?php echo DT_SKIN;?>image/list_img_on.gif" width="16" height="16" alt="图片列表" align="absmiddle"/>&nbsp;
			<?php } else { ?>
			<img src="<?php echo DT_SKIN;?>image/list_img.gif" width="16" height="16" alt="图片列表" align="absmiddle" class="c_p" onclick="$('list').value=1;$('fsearch').submit();"/>&nbsp;
			<?php } ?>
			<?php if($list == 0) { ?>
			<img src="<?php echo DT_SKIN;?>image/list_mix_on.gif" width="16" height="16" alt="图文列表" align="absmiddle"/>&nbsp;
			<?php } else { ?>
			<img src="<?php echo DT_SKIN;?>image/list_mix.gif" width="16" height="16" alt="图文列表" align="absmiddle" class="c_p" onclick="$('list').value=0;$('fsearch').submit();"/>&nbsp;
			<?php } ?>
			</td>
			</tr>
			</table>
...
<div class="list" id="item_<?php echo $t['itemid'];?>">
	...
	<tr align="center">
		...
		<td width="10"> </td>
		<td align="left">
			<h3><span class="f_r"><?php if($t['vip']) { ?> <img src="<?php echo DT_SKIN;?>image/vip.gif"/> <img src="<?php echo DT_SKIN;?>image/vip_<?php echo $t['vip'];?>.gif"/><?php } ?></span><a href="<?php echo $t['linkurl'];?>" target="_blank"><?php echo $t['title'];?></a></h3>
			<ul>
...
<td valign="top" width="20%" id="item_<?php echo $t['itemid'];?>">
	...
	<tr align="center">
	<td class="thumb"><a href="<?php echo $t['linkurl'];?>" target="_blank"><img src="<?php echo imgurl($t['thumb'], 1);?>" width="80" height="80" alt="" onmouseover="img_tip(this, this.src);" onmouseout="img_tip(this, '');" class="bd"/></a></td>
	</tr>
	<tr align="center">
	<td class="thumb">
	<ul>
	...
	<div style="padding:5px 0 0 0;">
		<?php if($t['unit'] && $t['price']>0) { ?>
			<span class="f_red"><strong class="px14"><?php echo $t['price'];?></strong>/<?php echo $t['unit'];?></span><br/>
			<a href="<?php echo $path;?><?php echo rewrite('order.php?itemid='.$t['itemid']);?>" class="l"><img src="<?php echo DT_SKIN;?>image/buy.gif" title="订购 <?php echo $t['minamount'];?><?php echo $t['unit'];?>起订" style="border:none;"/></a>
		<?php } else { ?>
		<span class="f_gray">面议</span><br/>
	...
	<ul>
	<li><?php if($t['vip']) { ?><img src="<?php echo DT_SKIN;?>image/vip.gif" alt="级别：<?php echo $t['vip'];?>" style="border:none;" align="absmiddle"/>&nbsp;<?php } ?><a href="<?php echo userurl($t['username']);?>" target="_blank"><?php echo $t['company'];?></a></li>
	</ul>
	</td>
	</tr>
	<tr align="center">
...
<?php if($pages) { ?><div class="pages"><?php echo $pages;?></div><?php } ?>
		...
		<?php } else { ?>
		<?php include template('list-sell', 'tag');?>
		<?php } ?>
		<div class="tool">
			<table>
			<tr height="30">
			...
			<td align="right"><img src="<?php echo DT_SKIN;?>image/arrow.gif" width="17" height="12" alt=""/> <a href="javascript:window.scrollTo(0,0);">返回顶部</a>&nbsp;&nbsp;&nbsp;&nbsp;<img src="<?php echo DT_SKIN;?>image/arrow.gif" width="17" height="12" alt=""/> <a href="<?php echo $MOD['linkurl'];?>search.php">重新搜索</a>&nbsp;</td>
			</tr>
			</table>
		</div>
		</form>
